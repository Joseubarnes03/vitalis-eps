...
const MEDICOS_OPTOMETRIA = [
  'Dr. Juan Pérez - Especialista en Retina',
  'Dra. Ana González - Especialista en Cataratas',
  'Dr. Carlos Rodríguez - Especialista en Glaucoma',
  'Dra. Laura Martínez - Especialista en Córnea'
];

export default function AgendaCitaOptometria({ navigation }) {
  ...
  const validarFormulario = () => {
    const { dia, hora, nombre, apellido, cedula, correo, medico, motivo } = form;
    const errores = [];
    ...
    if (!medico) errores.push('Oftalmólogo es requerido');
  ...
  const handleSubmit = async () => {
    ...
    if (success) {
      Alert.alert(
        'Cita Confirmada', 
        `Tu cita de optometría ha sido agendada para el ${form.dia} a las ${form.hora} con ${form.medico.split(' - ')[0]}`,
        [
          { 
            text: 'OK', 
            onPress: () => {
  ...
  const exportarCitas = async () => {
    try {
      const citas = await getCitas();
      const citasOptometria = citas.filter(c => c.tipoCita === 'Optometría');

      if (!citasOptometria.length) {
        Alert.alert('No hay citas', 'No hay citas de optometría para exportar.');
        return;
      }

      const contenido = citasOptometria.map(cita => 
        `Paciente: ${cita.nombre} ${cita.apellido}
Cédula: ${cita.cedula}
Fecha: ${cita.dia} ${cita.hora}
Oftalmólogo: ${cita.medico}
Motivo: ${cita.motivo}
Antecedentes: ${cita.antecedentes || 'No especificado'}
Visitado antes: ${cita.visitado ? 'Sí' : 'No'}
Correo: ${cita.correo}
Fecha registro: ${new Date(cita.fechaCreacion).toLocaleString()}\n`
      ).join('\n');

      const fileUri = FileSystem.documentDirectory + 'citas_optometria.txt';
      await FileSystem.writeAsStringAsync(fileUri, contenido);

      await Sharing.shareAsync(fileUri, {
        mimeType: 'text/plain',
        dialogTitle: 'Exportar citas de optometría',
...
Oftalmólogo: ${form.medico || 'No seleccionado'}
Motivo: ${form.motivo || 'No especificado'}
Antecedentes: ${form.antecedentes || 'No especificados'}
    ...
    >
      <View style={styles.container}>
        ...
        <View style={styles.formContainer}>
          ...
          <TextInput
            placeholder="Antecedentes oculares (opcional)"
            style={[styles.input, styles.multilineInput, activeField === 'antecedentes' && styles.activeInput]}
            value={form.antecedentes}
            onChangeText={(text) => handleChange('antecedentes', text)}
            multiline
            numberOfLines={3}
          ...
          {/* Selección de médico */}
          <Text style={styles.sectionTitle}>Oftalmólogo</Text>
          ...
          >
            ...
            <Text style={[styles.medicoText, !form.medico && styles.placeholderText]}>
              {form.medico || 'Seleccionar oftalmólogo'}
            </Text>
